...
FIRDocumentSnapshot *FSTTestDocSnapshot(const absl::string_view path,
                                        ...
                                        BOOL hasMutations,
                                        BOOL fromCache) {
  FSTDocument *doc = data ? FSTTestDoc(path, version, data, hasMutations ? FSTDocumentStateLocalMutations : FSTDocumentStateSynced) : nil;
...
FIRQuerySnapshot *FSTTestQuerySnapshot(
  ...
  for (NSString *key in oldDocs) {
    oldDocuments =
        [oldDocuments documentSetByAddingDocument:FSTTestDoc(util::StringFormat("%s/%s", path, key),
                                                             1, oldDocs[key], hasPendingWrites ? FSTDocumentStateLocalMutations : FSTDocumentStateSynced)];
  }
  ...
  for (NSString *key in docsToAdd) {
    FSTDocument *docToAdd =
        FSTTestDoc(util::StringFormat("%s/%s", path, key), 1, docsToAdd[key], hasPendingWrites ? FSTDocumentStateLocalMutations : FSTDocumentStateSynced);
